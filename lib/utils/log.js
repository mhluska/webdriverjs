...
log.result = function(result, data, callback) {
    ...
    if (result.status === 0) {
        ...
        error = {
            status: result.status,
            type: errorCodes[result.status] ? errorCodes[result.status].id : 'unknown',
            message: errorCodes[result.status] ? errorCodes[result.status].message : 'unknown',
            orgStatusMessage: result.value.message
        };
    ...
    if (callback) {
        callback(require('./makeError')(error, data),result);
    }
};
